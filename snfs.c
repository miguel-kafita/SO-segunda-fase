   ...
   int* ressz)
{
   ...
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.ping);
   ...
   // handle request, prepare responde string
   strcpy(res->body.ping.msg, "hello");
...
{
   // get input arguments
   char* srcpath = req->body.copy.srcpathname;
   char* tgtpath = req->body.copy.tgtpathname;
   ...
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.copy);
   ...
   // handle request
   if (fs_copy(FS,srcpath,tgtpath) == 0) {
     res->status = RES_OK;
   }
}
